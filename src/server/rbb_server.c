...
static int rbb_add_tms_seq(FILE* fp, int region_cnt, struct jtag_region* region, unsigned char* tms_input, unsigned char* tdi_input)
{
	/* clear tms buffer */
	...
	fprintf(fp, "region %d st %s -> %s ", region_cnt,
			tap_state_name(region->begin_state), tap_state_name(region->end_state));
	fprintf(fp, "start %d end %d size %d\n", region->begin, region->end, region->end - region->begin);

	int region_size = (region->end - region->begin + 7) / 8;
	fprintf(fp, "TMS: ");
	for (int byte_index = 0; byte_index < region_size; byte_index++) {
		fprintf(fp, "%02x ", region->tms_buffer[byte_index]);
	}
	fprintf(fp, "\n");
...
static int rbb_add_tdi_seq(FILE* fp, int region_cnt, struct jtag_region* region, unsigned char* tdi_input, unsigned char* read_input)
{
	...
	fprintf(fp, "region %d st %s -> %s ", region_cnt,
			tap_state_name(region->begin_state), tap_state_name(region->end_state));
	fprintf(fp, "start %d end %d size %d\n", region->begin, region->end, region->end - region->begin);
	fprintf(fp, "next region st %s\n", tap_state_name(region->next_state));
			
	int region_size = (region->end - region->begin + 7) / 8;
	fprintf(fp, "TDI: ");
	for (int byte_index = 0; byte_index < region_size; byte_index++) {
		fprintf(fp, "%02x ", region->tdi_buffer[byte_index]);
	}
	fprintf(fp, "\n");
						  ...
						  unsigned char* tms_input, unsigned char* tdi_input, unsigned char* read_input)
{
	...
	for (i = 0; i < service->region_count; i++) {
		if(service->regions[i].is_tms) {
			...
			rbb_add_tms_seq(fp_tdi, i, &(service->regions[i]), tms_input, tdi_input);
			...
			rbb_add_tdi_seq(fp_tdi , i, &(service->regions[i]), tdi_input, read_input);
		}
	}
...
static int rbb_input(struct connection *connection)
{
	...
	// unsigned char command_buffer[RBB_BUFFERSIZE + 1];
