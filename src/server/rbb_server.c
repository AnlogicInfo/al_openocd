				...
				const uint8_t *read_bits, struct rbb_service *service)
{
	...
	service->regions[0].beginstate = cur_state;
	...
	for (int i = 0; i < bits; i++) {
		...
		if ((cur_state != TAP_DRSHIFT && new_state == TAP_DRSHIFT) ||
			...
			next_region->beginstate = new_state;
...
static int rbb_input(struct connection *connection)
{
	...
	for (i = 0; i < service->region_count; i++) {
		...
		if (service->regions[i].is_tms) { /* Handle TAP state change */
			...
			// tdo_buf_size[tdi_buffer_count] = service->regions[i].end - service->regions[i].begin;
...
// #endif
	...
	// if (fp_tdo != NULL) {
	// 	for(k = 0; k < (int) tdi_buffer_count; k++){
	// 		for(tdo_byte = 0; tdo_byte < (tdo_buf_size[k] + 8 - 1) / 8; tdo_byte++)
	// 		{
	// 			total_tdo_bytes = tdo_byte + 1;
	// 			fprintf(fp_tdo, "%x ", tdo_buffer[k][tdo_byte]);
	// 			if((tdo_byte + 1) % 16 == 0)
	// 				fprintf(fp_tdo, "\n");
	// 		}
	// 	}
	// 	fprintf(fp_tdo, "\n");
	// 	fprintf(fp_tdo, "total tdo bytes %d\n", total_tdo_bytes);
	// }
