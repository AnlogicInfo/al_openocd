...
static void analyze_bitbang(const uint8_t *tms, int total_bits,
				const uint8_t *read_input, struct rbb_service *service)
{
	int shift_pos = 0;
	...
	for (int i = 0; i < total_bits; i++) {
		...
		if ((cur_state != TAP_DRSHIFT && new_state == TAP_DRSHIFT) ||
			...
			LOG_DEBUG("st %s -> %s, pos %d", tap_state_name(cur_state), tap_state_name(new_state), i);
...
static int rbb_read_sel(struct jtag_region* region, unsigned char* read_input, uint8_t* read_mask, size_t* tdo_size)
{
	int i;
	int in_byte_index, in_bit_index;
	int out_byte_index, out_bit_index;
...
static int rbb_jtag_drive(struct rbb_service *service, unsigned char* tms_input, unsigned char* tdi_input,
						  unsigned char* read_input, unsigned char* read_output, size_t read_bits)
{
	size_t i, j;
	...
	for (i = 0; i < service->region_count; i++) {
		if(service->regions[i].is_tms) {
			...
			rbb_read_sel(&(service->regions[i]), read_input, read_mask_buffer[tdi_buffer_count], &(tdo_size[tdi_buffer_count]));
			tdi_buffer_count ++;
		}
	...
	LOG_DEBUG("gen read out");
	for(i = 0; i < tdi_buffer_count; i++) {
	...
	if (read_p != read_bits) {
		LOG_ERROR("read_p %lld read bits %lld", read_p, read_bits);
		retval = ERROR_FAIL;
	}
	// assert(read_p == read_bits);
	return retval;
}
...
static int rbb_input(struct connection *connection)
{
	...
	LOG_INFO("rbb analyze");
	analyze_bitbang((uint8_t *) tms_input, total_bits, (uint8_t *)read_input, service);

	LOG_INFO("region count %d", (int) (service->region_count));
	LOG_INFO("jtag drive");
	retval = rbb_jtag_drive(service, tms_input, tdi_input,
							read_input, send_buffer, total_read_bits);
	if(retval != ERROR_OK)
		return retval;
	LOG_INFO("connection write");
