 ...
 */
void
vexriscv_semihosting_init (struct target *target)
{
...
static int
vexriscv_create_reg_list (struct target *target)
{
  ...
  for (i = 0; i < ARRAY_SIZE (vexriscv_init_reg_list); i++)
	...
	&& (vexriscv_init_reg_list[i].csr_num > vexriscv->largest_csr))
...
static int
vexriscv_target_create (struct target *target, Jim_Interp * interp)
{
...
static int
vexriscv_execute_jtag_queue (struct target *target)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  if (vexriscv->useTCP)
    return 0;
...
int
vexriscv_write_regfile (struct target *target, bool execute, uint32_t regId,
			uint32_t value)
{
  assert (regId <= 32);
  if (value & 0xFFFFF800)
...
static int
vexriscv_get_core_reg (struct reg *reg)
{
  ...
  if (!reg->valid)
      ...
      else if (vexriscv_reg->is_csr)
	{
	  ...
	  vexriscv_pushInstruction (target, false, 0 | ((vexriscv_reg->csr_num & 0x1fff) << 20) | (0 << 15)	// rs1: x0
				    | (2 << 12)	// CSRRW
				    | (1 << 7)	// rd: x1
				    | (0x73 << 0)	// SYSTEM
	    );
...
static int
vexriscv_set_core_reg (struct reg *reg, uint8_t * buf)
{
  struct vexriscv_core_reg *vexriscv_reg = reg->arch_info;
  struct target *target = vexriscv_reg->target;
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  ...
  if (vexriscv_reg->is_csr)
    {
      ...
      vexriscv_pushInstruction (target, false, 0 | ((vexriscv_reg->csr_num & 0x1fff) << 20) | (1 << 15)	// rs1: x1
				| (1 << 12)	// CSRRW
				| (0 << 7)	// rd: x0
				| (0x73 << 0)	// SYSTEM
	);
...
vexriscv_build_reg_cache (struct target *target)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
...
static void
vexriscv_set_instr (struct target *target, uint32_t new_instr)
{
  struct scan_field field;
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  struct jtag_tap *tap = target->tap;
  if (!target->tap->bypass
      && buf_get_u32 (target->tap->cur_instr, 0,
		      target->tap->ir_length) == new_instr)
    return;
...
static void
vexriscv_yaml_ignore_block (yaml_parser_t * parser)
{
  yaml_token_t token;
  int32_t level = 0;
  while (1)
...
static void
vexriscv_parse_debugReport (yaml_parser_t * parser,
	  ...
	  if (strcmp
	      ...
	      while (1)
		{
		  ...
		  switch (token.type)
		    {
		    case YAML_SCALAR_TOKEN:
		      target->hardwareBreakpointsCount =
			atoi ((char *) token.data.scalar.value);
		      return;
		      break;
		    default:
		      break;
...
static void
vexriscv_parse_busInfo (yaml_parser_t * parser, struct BusInfo *busInfo)
{
  yaml_token_t token;
  ...
  while (1)
	...
	{
	case YAML_SCALAR_TOKEN:
	  if (strcmp ((char *) token.data.scalar.value, "flushInstructions")
	      == 0)
	    {
	      ...
	      while (1)
		{
		  ...
		  switch (token.type)
		    {
		    case YAML_SCALAR_TOKEN:
		      busInfo->flushInstructions[busInfo->
...
static int
vexriscv_parse_cpu_file (struct command_context *cmd_ctx,
			 struct target *target)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
...
static int
vexriscv_init_target (struct command_context *cmd_ctx, struct target *target)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  ...
  if (vexriscv->useTCP)
	...
	LOG_ERROR ("Unrecognized network protocol defined");
      ...
      if (connect
	  (vexriscv->clientSocket, (struct sockaddr *) &serverAddr,
	   addr_size) != 0)
	{
...
static int
vexriscv_arch_state (struct target *target)
{
...
static int
vexriscv_is_halted (struct target *target, uint32_t * halted)
{
  uint32_t flags;
  int error;
  if ((error =
       vexriscv_readStatusRegister (target, true, &flags)) != ERROR_OK)
    {
...
static int
vexriscv_is_running (struct target *target, uint32_t * running)
{
  uint32_t flags;
  int error;
  if ((error =
...
static int
vexriscv_flush_bus (struct target *target, struct BusInfo *busInfo)
{
  int error;
  if (!busInfo)
    return ERROR_OK;
...
static int
vexriscv_flush_caches (struct target *target)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  int error;
  if (vexriscv->iBus != 0)
...
static int
vexriscv_save_context (struct target *target)
{
  int error;
  LOG_DEBUG ("-");
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
...
static void
vexriscv_cpu_write_pc (struct target *target, bool execute, uint32_t value)
{
...
static int
vexriscv_restore_context (struct target *target)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  LOG_DEBUG ("-");
...
static int
vexriscv_debug_entry (struct target *target)
{
  int error;
  LOG_DEBUG ("-");
...
static int
vexriscv_halt (struct target *target)
{
  int error;
 ...
 */
int
vexriscv_semihosting (struct target *target, int *retval)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  ...
  pc =
    buf_get_u32 ((uint8_t *) vexriscv->core_cache->reg_list[32].value, 0, 32);
  ...
  /*int result = */ vexriscv_read_memory (target, pc - 4, sizeof (uint16_t),
					  6, tmp);
  ...
  if (!semihosting->hit_fileio)
    {
      ...
      r0 =
	buf_get_u32 ((uint8_t *) vexriscv->core_cache->reg_list[10].value, 0,
		     32);

      vexriscv_get_core_reg (&vexriscv->core_cache->reg_list[11]);
      r1 =
	buf_get_u32 ((uint8_t *) vexriscv->core_cache->reg_list[11].value, 0,
		     32);
  ...
  if (semihosting->is_resumable && !semihosting->hit_fileio)
      ...
      else if (target->debug_reason == DBG_REASON_SINGLESTEP)
	{
	  ...
	  pc += 4;
	  vexriscv_set_core_reg (&vexriscv->core_cache->reg_list[32],
				 (uint8_t *) & pc);
	  return 1;
	}
      else
...
#include <stdlib.h>
static int
vexriscv_poll (struct target *target)
{
  int retval;
...
static int
vexriscv_assert_reset (struct target *target)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  int error;
...
static int
vexriscv_deassert_reset (struct target *target)
{
  int error;
  LOG_DEBUG ("vexriscv_deassert_reset\n");

  error =
    vexriscv_writeStatusRegister (target, true, vexriscv_FLAGS_RESET_CLEAR);
  if (error != ERROR_OK)
    return error;
...
static int
vexriscv_network_read (struct vexriscv_common *vexriscv, void *buffer,
		       size_t count)
{
  if (vexriscv->networkProtocol == NP_IVERILOG)
...
static int
vexriscv_network_write (struct vexriscv_common *vexriscv, int is_read,
			uint32_t size, uint32_t address, uint32_t data)
{
  if (vexriscv->networkProtocol == NP_IVERILOG)
    {
      uint8_t buffer[10];
...
static void
vexriscv_memory_cmd (struct target *target, uint32_t address, uint32_t data,
		     int32_t size, int read)
{
  ...
  if (ignoreWidth < 0)
    {
      LOG_ERROR
	("To support VexRiscv jtag header and multiple tap on the jtag chain, you need to provision more 'jtagIgnoreWidth'\n");
      exit (1);
    }
...
static void
vexriscv_read_rsp (struct target *target, uint8_t * value, uint32_t size)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  struct jtag_tap *tap = target->tap;
  ...
  if (!vexriscv->useTCP)
  ...
  else
    {
      uint32_t buffer;
      int bytes_read =
	vexriscv_network_read (vexriscv, &buffer, sizeof (buffer));
      if (bytes_read == 4)
	{
	  //value[0] = 1;
...
static int
vexriscv_read_memory (struct target *target, target_addr_t address,
...
int
vexriscv_mem_access_comp (const void *elem1, const void *elem2)
{
...
static int
vexriscv_write_memory (struct target *target, target_addr_t address,
		       uint32_t size, uint32_t count, const uint8_t * buffer)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  LOG_DEBUG ("Writing memory at physical address 0x%" PRIx32
	     "; size %" PRId32 "; count %" PRId32, (uint32_t) address, size,
	     count);
  ...
  if (size == 4 && count > 4)
	...
	{
	  if (vexriscv_write_memory
	      (target, address, size, numAddressReg * 4096 / size, buffer))
	    return ERROR_FAIL;
	  if (vexriscv_write_memory
	      (target, address + 4096 * numAddressReg, size,
	       count - 4096 / size * numAddressReg,
	       buffer + 4096 * numAddressReg))
	    return ERROR_FAIL;
	  return ERROR_OK;
	}
...
static int
vexriscv_pushInstruction (struct target *target, bool execute,
...
static int
vexriscv_setHardwareBreakpoint (struct target *target, bool execute,
				uint32_t id, uint32_t enable, uint32_t pc)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
...
static int
vexriscv_writeStatusRegister (struct target *target, bool execute,
			      uint32_t value)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
...
static int
vexriscv_readStatusRegister (struct target *target, bool execute,
			     uint32_t * value)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
...
static int
vexriscv_readInstructionResult32 (struct target *target, bool execute,
				  uint8_t * value)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
...
static int
vexriscv_readInstructionResult16 (struct target *target, bool execute,
				  uint8_t * value)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  vexriscv_memory_cmd (target, vexriscv->dbgBase + 4, 0, 4, 1);
...
static int
vexriscv_readInstructionResult8 (struct target *target, bool execute,
				 uint8_t * value)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
  vexriscv_memory_cmd (target, vexriscv->dbgBase + 4, 0, 4, 1);
...
static int
vexriscv_read16 (struct target *target, uint32_t address, uint16_t * data)
{
  return vexriscv_read_memory (target, address, 2, 1, (uint8_t *) data);
}

static int
vexriscv_write16 (struct target *target, uint32_t address, uint16_t data)
{
 ...
 */
static int
vexriscv_semihosting_setup (struct target *target, int enable)
{
...
static int
vexriscv_semihosting_post_result (struct target *target)
{
  struct vexriscv_common *vexriscv = target_to_vexriscv (target);
...
static int
vexriscv_get_gdb_reg_list (struct target *target, struct reg **reg_list[],
			   int *reg_list_size,
...
static int
vexriscv_add_breakpoint (struct target *target, struct breakpoint *breakpoint)
{
  uint32_t data;
  ...
  if (breakpoint->type == BKPT_SOFT)
      ...
      if ((data & 3) == 3)
	{
	  ...
	  if (retval != ERROR_OK)
	    {
	      LOG_ERROR ("Error while writing vexriscv_TRAP_INSTR at 0x%08"
	  ...
	  if (retval != ERROR_OK)
	    {
	      LOG_ERROR ("Error while writing vexriscv_TRAP_INSTR at 0x%08"
			 PRIx32, (uint32_t) breakpoint->address + 2);
	      return retval;
	    }
...
static int
vexriscv_remove_breakpoint (struct target *target,
...
static int
vexriscv_resume_or_step (struct target *target, int current,
  ...
  if (!debug_execution)
    {
      ...
      LOG_DEBUG ("Target resumed at 0x%08" PRIx32,
		 buf_get_u32 ((uint8_t *) vexriscv->regs->pc.value, 0, 32));
    }
  else
    {
...
static int
vexriscv_resume (struct target *target, int current,
...
static int
vexriscv_step (struct target *target, int current,
	       target_addr_t address, int handle_breakpoints)
{
  return vexriscv_resume_or_step (target, current, address,
...
static int
vexriscv_examine (struct target *target)
{
      ...
      else
	{
	  if (!halted)
		...
		return ERROR_FAIL;
		  ...
		  || buffer[2] != 0xFFFFF000 || buffer[3] != 0xABCDE000)
		{
		  LOG_ERROR ("!!!");
...
static int
vexriscv_soft_reset_halt (struct target *target)
{
  int error;
  LOG_DEBUG ("-");
  ...
  error =
    vexriscv_writeStatusRegister (target, true, vexriscv_FLAGS_RESET_SET);
  if (error != ERROR_OK)
    {
      LOG_ERROR ("Error while soft_reset_halt the CPU");
      return error;
...
static int
vexriscv_add_watchpoint (struct target *target, struct watchpoint *watchpoint)
{
...
static int
vexriscv_remove_watchpoint (struct target *target,
...
static int
vexriscv_run_and_wait (struct target *target, target_addr_t entry_point,
		       int timeout_ms)
{
  int retval;
  //struct vexriscv_common *vexriscv = target_to_vexriscv(target);
  ...
  if ((retval =
       vexriscv_writeStatusRegister (target, true,
				     vexriscv_FLAGS_HALT_CLEAR)) != ERROR_OK)
    {
      LOG_ERROR ("Error while unstalling the CPU");
      return retval;
    }
...
int
vexriscv_run_algorithm (struct target *target, int num_mem_params,
			...
			int timeout_ms, void *arch_info)
{
  ...
  for (int i = 0; i < num_reg_params; i++)
    {
      struct reg *reg =
	register_get_by_name (vexriscv->core_cache, reg_params[i].reg_name,
			      0);
