...
static const struct fespi_target target_devices[] = {
	...
	{ "Nuclei SOC SPI Flash", 0x13090a6d, 0xe0050000 },
	{ NULL, 0, 0 }
};
...
static void fespi_write_sync_params_priv(struct flash_loader *loader)
{
	struct fespi_flash_bank *fespi_info = loader->dev_info;
	buf_set_u64(loader->reg_params[5].value, 0, loader->xlen,
					fespi_info->dev->pprog_cmd | (fespi_info->dev->size_in_bytes > 0x1000000 ? 0x100 : 0));	
}
...
{
	struct fespi_flash_bank *fespi_info = bank->driver_priv;
	struct flash_loader *loader = &fespi_info->loader;
	int retval = ERROR_OK;

	loader->work_mode = SYNC_TRANS;
	loader->block_size = fespi_info->dev->pagesize;
	loader->image_size = count;
	loader->param_cnt = 6;
	loader->set_params_priv = fespi_write_sync_params_priv;
