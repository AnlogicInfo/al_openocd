...
{
    // uint8_t wr_seq[8] = {SPIFLASH_WRITE_ENABLE, 0};
    uint8_t wr_en = SPIFLASH_WRITE_ENABLE;
    uint8_t sr;
    // memcpy(&wr_seq[1], cmd, len);
...
static int dwcssi_checksum_x1(struct flash_bank *bank, target_addr_t address, uint32_t count, uint32_t *crc)
{
    ...
    loader->block_size = X1_PAGE_SIZE; // controller rx/tx fifo size limit
...
//     dwcssi_disable(bank);
//     dwcssi_config_CTRLR0(bank, DFS_BYTE, SPI_FRF_X1_MODE, TX_ONLY);
//     dwcssi_config_TXFTLR(bank, 0, len+3); //1+1+4
//     dwcssi_enable(bank);
...
//     // page_size = driver_priv->dev->pagesize;
//     page_size = X1_PAGE_SIZE; // tx fifo size is 128
// 	while (count > 0) {
