...
static int target_async_algorithm_init_fifo(struct target *trans_target,  uint32_t buffer_start, uint32_t buffer_size, uint32_t block_size, struct async_fifo *fifo)
{
	...
	target_read_u32(trans_target, fifo->wp_addr, &wp_result);

	retval = target_write_u32(trans_target, fifo->rp_addr, fifo->rp);
	if(retval != ERROR_OK)
		return retval;
    ...
    const uint8_t *buffer, int count, uint32_t block_size, struct ping_pong_fifo *pp)
{
    ...
    while (count > 0) {
        ...
        count    -= this_blocks;
        pp->prod_idx ^= 1;
		LOG_INFO("pp fifo trans batch %u, %X blocks, remain %X blocks", ++batch_cnt, this_blocks, count);
		...
		uint32_t entry_point, uint32_t exit_point, void *arch_info)
{
	...
	LOG_INFO("buffer start %x size %x", buffer_start, buffer_size);
